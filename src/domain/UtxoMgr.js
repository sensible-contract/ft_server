...
class UtxoMgr {
  ...
  static async adjustUtxos() {
    ...
    if (this.utxos.length >= app.get("ftConfig").minSplit) {
    ...
    const unitSatoshis = app.get("ftConfig").unitSatoshis;
    ...
    for (let i = 0; i < toSplitCount; i++) {
      ...
      if (
        leftSatoshis < Math.ceil(tx._estimateSize() * app.get("ftConfig").feeb)
      ) {
    ...
    tx.change(ScriptHelper.dummyAddress).fee(
      Math.max(
        Math.ceil(tx._estimateSize() * app.get("ftConfig").feeb),
        MIN_FEE
      )
    );
