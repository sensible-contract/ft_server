...
const Genesis = buildContractClass(
  ScriptHelper.loadDesc("tokenGenesis_desc.json")
);
const Token = buildContractClass(ScriptHelper.loadDesc("token_desc.json"));
...
const RouteCheck = buildContractClass(
  ScriptHelper.loadDesc("tokenRouteCheck_desc.json")
);
const UnlockContractCheck = buildContractClass(
...
TokenUtil.createGenesisContract = function (
  issuerPubKey,
  rabinPubKeyArray,
  tokenName,
  tokenSymbol,
...
TokenUtil.createGenesis = function (
  ...
  const hashData = bsv.crypto.Hash.sha256ripemd160(
    inputPrivKey.publicKey.toBuffer()
  );
  const sig = tx.inputs[0].getSignatures(
    tx,
    inputPrivKey,
    0,
    sigtype,
 ...
 * @param utxos {Object} the input utxos
...
TokenUtil.createToken = function (
  ...
  const bsvInputPrivKey = issuerPrivKey;
  ...
  let lockingScript = bsv.Script.fromBuffer(
    TokenProto.getNewGenesisScript(genesisScript.toBuffer(), tokenID)
  );
  tx.addOutput(
    new bsv.Transaction.Output({
  ...
  const bsvInputAmount = utxos.reduce((pre, cur) => pre + cur.satoshis, 0);
  const changeSatoshis =
    inputAmount + bsvInputAmount - outputSatoshis * 2 - fee;
  if (changeSatoshis > 0) {
    tx.addOutput(
      new bsv.Transaction.Output({
        script: bsv.Script.buildPublicKeyHashOut(changeAddress),
  ...
  const preimage = getPreimage(
    ...
    genesisScript.toASM(),
    inputAmount,
    (inputIndex = genesisTxOutputIndex),
    (sighashType = sigtype)
  );
  ...
  const hashData = bsv.crypto.Hash.sha256ripemd160(
    bsvInputPrivKey.publicKey.toBuffer()
  );
...
TokenUtil.createTokenTransfer = function (
  ...
  let preimage = getPreimage(
    tx,
    scriptTx.outputs[0].script.toASM(),
    scriptTx.outputs[0].satoshis,
    (inputIndex = scriptInputIndex),
    (sighashType = sigtype)
  );
  const unlockingScript = routeCheckCode
    .unlock(
      new SigHashPreimage(toHex(preimage)),
      tokenInputLen,
